...
namespace Austin.PBConsoleFrontend
{
    class Program
    {
        ...
        static Dictionary<Keys, byte> consoleKeyToScanCodes = new Dictionary<Keys, byte>();
        static Form1 frm;
        ...
        private static byte FB_COLOR = 0;

        static void Main(string[] args)
        {
            loadKeys();

            iMem = new Compiler().Compile(new System.IO.StringReader(Properties.Resources.Prog_Rom));
            cpu = new Cpu(iMem);


            cpu.RegisterInput(0x20, () => 0); // buttons
            cpu.RegisterInput(0x24, () => 0); // switches
